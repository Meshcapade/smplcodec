...
SMPLParamStructure = {
    ...
    SMPLVersion.SMPLH: PoseParameterSizes(body_pose=(22, 3),
                                          left_hand_pose=(15, 3),
    ...
    SMPLVersion.SUPR: PoseParameterSizes(body_pose=(22, 3),
                                          head_pose=(3,3),
                                          left_hand_pose=(15, 3),
                                          right_hand_pose=(15, 3) #,
                                          # left_foot_pose=(10, 3),
...
class SMPLCodec:
    ...
    def full_pose(self) -> ArrayLike:
        ...
        for field in fields(SMPLParamStructure[self.smpl_version]):
            ...
            if part_pose is None:
                part_pose = np.zeros(((count,) + getattr(SMPLParamStructure[self.smpl_version], field.name))) # merge tuples for shape
    ...
    @classmethod
    def from_amass_npz(cls, 
                       filename: str,
                       smpl_version_str: str ='smplx'):
        with closing(np.load(filename, allow_pickle=True)) as infile:
            ...
            mapped_dict = {
                ...
                'frame_count': int(in_dict.get('frameCount', in_dict['trans'].shape[0])), # expects trans to be not None if frameCount does not exist
                # for frame rate, different names seem to exist in AMASS (SMPLH: mocap_framerate, SMPLX: mocap_frame_rate?!)
                'frame_rate': float(in_dict.get('frameRate', in_dict.get('mocap_frame_rate', in_dict.get('mocap_framerate', 60.0)))), # default 60.0??
            }

            # check if pose parameters are stored separately
            if 'body_pose' in in_dict:
                mapped_dict['body_pose'] = np.concatenate((in_dict['root_orient'], in_dict['pose_body']), axis=-1).reshape(mapped_dict['frame_count'], -1, 3)
                
                if 'pose_jaw' in in_dict and 'pose_eye' in in_dict:
                    mapped_dict['head_pose'] = np.concatenate((in_dict['pose_jaw'], in_dict['pose_eye']), axis=-1).reshape(mapped_dict['frame_count'], -1, 3)
                
                if 'pose_hand' in in_dict:
                    mapped_dict['left_hand_pose'] = in_dict['pose_hand'][:, :int(in_dict['pose_hand'].shape[-1] / 2.)].reshape(mapped_dict['frame_count'], -1, 3)
                    mapped_dict['right_hand_pose'] = in_dict['pose_hand'][:, int(in_dict['pose_hand'].shape[-1] / 2.):].reshape(mapped_dict['frame_count'], -1, 3)
                ...
                for field in fields(joint_info):
                    ...
                    if num_params is not None:
                        mapped_dict[field.name] = in_dict['poses'][:, start_ind:start_ind+num_params].reshape((-1,) + getattr(joint_info, field.name))
        ...
        try:
            ...
            if self.frame_count is not None:
                ...
                for attr, shape in [("body_translation", (self.frame_count, 3))] + [
                    (field.name, ((self.frame_count,) + getattr(SMPLParamStructure[self.smpl_version], field.name))) for field in fields(SMPLParamStructure[self.smpl_version]) if getattr(SMPLParamStructure[self.smpl_version], field.name) is not None
                ]:
